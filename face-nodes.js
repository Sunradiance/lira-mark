...
window.LiraNodeFace = function (opts) {
  ...
  function speakLine(line, source) {
    ...
    if (!text) return;
    // Tilen typing/mic: show status only — NEVER move Lira's lips for his words
    if (!isLiraSource(source)) {
      if (source === 'you' || source === 'mic' || source === 'gremlin' || source === 'tilen') {
        var elYou = document.getElementById('liraReply');
        // don't clear her last line forever; just status
        if (statusEl) {
          statusEl.textContent = 'Tilen · ' + text.slice(0, 40) + (text.length > 40 ? '…' : '');
        }
        if (sayInput && source === 'you') {
          sayInput.classList.add('live');
          setTimeout(function () { sayInput.classList.remove('live'); }, 800);
        }
        return;
      }
      if (sayInput && document.activeElement === sayInput && Date.now() < typingUntil) return;
    }
    // Lira only from here
    var el = document.getElementById('liraReply');
    var short = text.length > 140 ? text.slice(0, 137) + '…' : text;
    if (el) el.textContent = short;
    if (sayInput) { sayInput.blur(); typingUntil = 0; sayInput.value = ''; }
    saying = text;
    sayPhase = 0;
    sayTimer = Math.max(2.4, text.length * 0.072);
    if (voiceOn) {
      // lips driven by TTS timestamps / onplay — don't fake-pump before audio
      speakWithAraVoice(text);
      if (statusEl) statusEl.textContent = 'me · ara · lips';
    } else {
      pumpLips({ open: 0.7, wide: 0.45, smile: 0.2, pucker: 0.1, roll: 0.08 }, 0.92);
      if (statusEl) statusEl.textContent = 'me · voice off · lips only';
    }
  }
  ...
  /** Browser blocks audio until a click — unlock on first gesture. */
  function unlockAudioOnce() {
    if (window.__liraAudioUnlocked) return;
    window.__liraAudioUnlocked = true;
    try {
      var a = new Audio();
      a.src = 'data:audio/mp3;base64,';
      var p = a.play();
      if (p && p.catch) p.catch(function () { /* ignore */ });
    } catch (e) { /* ignore */ }
    if (statusEl && voiceOn) statusEl.textContent = HOLOGRAM_BUILD + ' · audio unlocked';
  }
    ...
    if (location.protocol === 'file:' || location.hostname.endsWith('github.io')) {
      ...
      fetch('/api/health')
        .then(function (r) {
          ...
          voiceOn = true;
          syncVoiceButton();
          if (statusEl) statusEl.textContent = HOLOGRAM_BUILD + ' · ara ready · click once if silent';
        })
        .catch(function () {
          // Don't kill voice forever — server may still be waking. Keep voiceOn; TTS will fail soft.
          if (statusEl) statusEl.textContent = HOLOGRAM_BUILD + ' · server slow · click voice on + page';
        });
    }
    // First click/tap unlocks autoplay so Ara can be heard
    ['pointerdown', 'keydown', 'touchstart'].forEach(function (ev) {
      document.addEventListener(ev, unlockAudioOnce, { once: true, capture: true });
    });
    setupSpeechRec();
    connectSpeakStream();
    layout();
